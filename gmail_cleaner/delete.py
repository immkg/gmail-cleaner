...
from .auth import trash_email
from .analyze import load_data, matches_auto_delete
...
def choose_delete_strategy():
    ...
    while True:
        ...
        if choice in DELETE_STRATEGIES: return DELETE_STRATEGIES[choice]
...
def get_newsletter_candidates(df):
    newsletter_df = df[
        ...
        (df["body_text"].fillna("").str.contains(NEWSLETTER_REGEX, case=False, regex=True))
    ]
...
def get_promotion_candidates(df):
    promo_df = df[
        ...
        (df["snippet"].fillna("").str.contains(PROMOTION_REGEX, case=False, regex=True))
    ]
...
def get_cluster_candidates(df):
    CUSTOM_STOPWORDS = {"email", "dear", "thank", "thanks", "hi", "hello", "com", "www", "http", "https"}
    documents = df["subject"].fillna("")
    stop_words = sklearn_text.ENGLISH_STOP_WORDS.union(CUSTOM_STOPWORDS)
    vectorizer = TfidfVectorizer(stop_words=list(stop_words), max_features=10000, min_df=3, ngram_range=(1,2))
        ...
        try:
            if "-" in part:
                ...
                for i in range(int(start), int(end) + 1):
                    if 1 <= i <= max_index: indexes.add(i - 1)
            else:
                value = int(part)
                if 1 <= value <= max_index: indexes.add(value - 1)
        except ValueError:
            pass
    return indexes

def build_pattern_deletes(sender_df, candidates, selection):
    if selection == "q": raise KeyboardInterrupt()
...
def handle_promotion(sender, sender_count, df):
    sender_df = df[df["email"] == sender]
    score = sender_df["subject"].fillna("").str.contains(PROMOTION_REGEX, case=False, regex=True).mean()
...
def handle_subject_pattern(pattern, count, df):
    print(f"\nPATTERN\n{count:,} emails\n{pattern}")
    samples = df[df["subject"].fillna("").apply(normalize_subject) == pattern]["subject"].head(10)
    print("\nSamples")
...
def delete_gmail(ids, dry_run=False):
    ...
    with ThreadPoolExecutor(max_workers=DELETE_WORKERS) as executor:
        future_to_id = {executor.submit(trash_email, gid): gid for gid in ids_to_delete}
    ...
    if successful_ids:
        ...
        db_conn.executemany("UPDATE gmail_messages SET deleted = 1 WHERE id = ?", [(gid,) for gid in successful_ids])
        db_conn.commit()
        db_conn.close()
...
def handle_delete(delete_ids, skip=False, dry_run=False):
    ...
    if not skip:
        ...
        if action == "exit":
            return "exit"
        if action == "now":
            pending_delete_ids = delete_gmail(pending_delete_ids, dry_run=dry_run)
